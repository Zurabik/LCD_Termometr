...
#define RADIO_PRIM_TX	0
#define RADIO_PRIM_RX	1
...
typedef enum _radio_pipe {
        ...
        RADIO_PIPE_4 = 4,
        RADIO_PIPE_5 = 5,
        RADIO_PIPE_EMPTY = 7,   // FIFO пусто, когда номер канала в статус регистре равен 0b111.
} RADIO_PIPE;
...
typedef enum _radio_stdby2a_delay {
	RADIO_DELAY_DISABLE,
	RADIO_DELAY_ENABLE,
} RADIO_STDB2A_DELAY;
 ...
 */
uint8_t Radio_Transmit(radiopacket_t* payload, uint8_t* address, RADIO_TX_WAIT wait);
