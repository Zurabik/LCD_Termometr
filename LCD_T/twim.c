...
{
 	uint16_t n=0;
 	while((TWI_TransceiverBusy()) && (n<16000))   //ждем, когда TWI модуль освободится
 	{
 		n++;
 	}
...
/****************************************************************************
 Подождать освобождения TWI модуля
****************************************************************************/
volatile void TWI_Delay()
{
 	uint16_t n=0;
	 while((TWI_GetState()!=TWI_SUCCESS) && (n<10000) )
	 {
		 n++;
	 }                      
}
